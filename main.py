...
async def cmd_admin(message: Message):
    ...
    try:
        ...
        await message.answer(f"✅ {message.reply_to_message.from_user.mention_html()} has been promoted to Admin!", parse_mode="HTML")
...
async def cmd_unadmin(message: Message):
    ...
    try:
        ...
        await message.answer(f"❌ {message.reply_to_message.from_user.mention_html()} has been demoted to regular member.", parse_mode="HTML")
    except Exception: pass
...
async def cmd_muter(message: Message):
    ...
    try:
        ...
        await message.answer(f"🤫 {message.reply_to_message.from_user.mention_html()} now has permission to mute players!", parse_mode="HTML")
    except Exception: pass
...
async def cmd_unmuter(message: Message):
    ...
    try:
        ...
        await message.answer(f"❌ {message.reply_to_message.from_user.mention_html()}'s mute permission has been revoked.", parse_mode="HTML")
    except Exception: pass
...
async def cmd_moderator(message: Message):
    ...
    try:
        ...
        await message.answer(f"👑 {message.reply_to_message.from_user.mention_html()} has been appointed as Moderator!", parse_mode="HTML")
    except Exception: pass
...
async def cmd_unmoderator(message: Message):
    ...
    try:
        ...
        await message.answer(f"❌ {message.reply_to_message.from_user.mention_html()} lost moderator privileges.", parse_mode="HTML")
    except Exception: pass
...
async def cmd_main_admin(message: Message):
    ...
    try:
        await bot.promote_chat_member(
            chat_id=message.chat.id, user_id=target_user.id, is_anonymous=False,
            can_manage_chat=True, can_delete_messages=True, can_restrict_members=True,
            can_promote_members=True, can_invite_users=True, can_change_info=True,
            can_post_stories=True, can_edit_stories=True, can_delete_stories=True
        )
        await message.answer(f"👑 {target_user.mention_html()} guruhning **Asosiy Admini (.main)** etib tayinlandi!\n⚠️ *Xavfsizlik uchun anonimlik huquqi berilmadi.*", parse_mode="HTML")
    except Exception:
        await message.answer("❌ Botda ushbu huquqlarni berish uchun ruxsat yetarli emas.")
...
async def cmd_unmain_admin(message: Message):
    ...
    try:
        ...
        await message.answer(f"❌ {target_user.mention_html()} ning barcha bosh adminlik (.main) huquqlari bekor qilindi.", parse_mode="HTML")
    except Exception:
...
async def cmd_warn(message: Message):
    ...
    if warns_db[user_id] >= 3:
        ...
        try:
            ...
            ban_time = int((datetime.now() + timedelta(days=1)).timestamp())
            await bot.restrict_chat_member(chat_id=message.chat.id, user_id=user_id, permissions=permissions, until_date=ban_time)
            await message.answer(f"🚨 {message.reply_to_message.from_user.mention_html()} 3 ta ogohlantirish oldi va 24 soatga mut qilindi!", parse_mode="HTML")
        except Exception: pass
    else:
        await message.answer(f"⚠️ {message.reply_to_message.from_user.mention_html()} ogohlantirish oldi! Jami: {warns_db[user_id]}/3", parse_mode="HTML")
...
async def cmd_unwarn(message: Message):
    ...
    user_id = message.reply_to_message.from_user.id
    warns_db[user_id] = 0
    await message.answer(f"✅ {message.reply_to_message.from_user.mention_html()} ning ogohlantirishlari nolga tushirildi!", parse_mode="HTML")
...
async def cmd_ban(message: Message):
    ...
    try:
        ...
        await message.answer(f"🚷 {message.reply_to_message.from_user.mention_html()} klubdan haydaldi (Ban)!", parse_mode="HTML")
    except Exception: pass
...
async def cmd_unban(message: Message):
    ...
    try:
        ...
        await message.answer(f"🔓 {message.reply_to_message.from_user.mention_html()} bandan chiqarildi!", parse_mode="HTML")
    except Exception: pass
...
async def cmd_night(message: Message):
    ...
    try:
        ...
        await message.answer(f"🌃 **Shahar uyquga ketmoqda... (Tungi rejim YOQILDI)**\nChat {message.from_user.mention_html()} tomonidan qulflandi. Jimjitlik saqlang! 🤐", parse_mode="HTML")
    except Exception: pass
...
async def cmd_crush(message: Message):
    ...
    if message.from_user.id == message.reply_to_message.from_user.id:
        await message.answer(f"❤️ {lover} o'z-o'ziga oshiq bo'lib qoldi! Narsissizm ham kerak-da... 😂", parse_mode="HTML")
        return
    await message.answer(f"💘 Ohho, guruhimizda yangi juftlikmi?\n\n💞 {lover} kutilmaganda {target} ga oshiq bo'lib qoldi! 😍✨\nBaxtli bo'linglar! 🥳", parse_mode="HTML")
...
# ==================== MESSAGE TRACKER (ENG TAGDA TURISHI SHART) ====================
